...
export default class TelegramBot {
  ...
  private start() {
    this.bot.on(["/start"], async (msg) => {
      try {
        ...
        return this.bot.sendMessage(msg.from.id, "## Опа, адажи ", {
          parseMode: "markdown",
        });
      } catch (error) {
        console.error(error);
  ...
  private instLoginStep() {
    this.bot.on("text", async (msg) => {
      if (msg.entities && msg.entities.length > -1) return;
      try {
        ...
        if (!user.inst_login) {
          ...
          this.bot.sendMessage(
            msg.from.id,
            "Проверка займет максимум минутку\nОжидайте... ⌛",
            { parseMode: "markdown" }
          );
          const index = await this.iBot.isFollowed(msg.text.toLowerCase());
          if (index && isDocument(user.step)) {
            await user.updateOne({
              $set: { inst_login: msg.text, step: user.step.next },
            });
            const nextStep = user.step.next;
            if (isDocument(nextStep)) return this.sendStep(user, nextStep);
  ...
  private sendStep(user: User, step: Step, error: boolean = false): void {
    switch (step.attach_type) {
      case "file":
        this.bot.sendMessage(user.tg_id, step.message, {
          parseMode: "markdown",
        });
        return this.bot.sendDocument(
          user.tg_id,
          path.resolve(__dirname, "../../../public", step.attach_file)
        );
      default:
        if (error)
          return this.bot.sendMessage(user.tg_id, step.error_message, {
            parseMode: "markdown",
          });
        return this.bot.sendMessage(user.tg_id, step.message, {
          parseMode: "markdown",
        });
    }
  }
